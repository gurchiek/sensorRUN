function [ fc, side, rshankfilt, lshankfilt, sacrumfilt] = sensorRUNv02_footContact_algo1(rshank,lshank,sacrum,fs)
%Reed Gurchiek, 2019
%
%-----------------------------DESCRIPTION----------------------------------
%
...
%
%   rshankfilt, lshankfilt, sacrumfilt:
%       1xn double array, filtered resultant accelerometer data from the right
%       shank, left shank, and sacrum, used to find foot contacts
%
%---------------------------REVISION HISTORY-------------------------------
%
% Jess Zendler, 2021: 
%   - added return of filtered rshank, lshank, sacrum signals
%
%--------------------------------------------------------------------------
%% sensorRUNv02_footContact_algo1
...
% lowpass
rshankfilt = bwfilt(rshank,domf,fs,'low',4);
lshankfilt = bwfilt(lshank,domf,fs,'low',4);
sacrumfilt = bwfilt(sacrum,domf,fs,'low',4);

% get peaks
[~,ipk] = findpeaks(sacrumfilt);
...
for p = 1:length(ipk)-1
    ...
    % get peak r and l
    [right_peaks,iright_peaks] = findpeaks(rshankfilt(ipk(p):ipk(p+1)));
    [left_peaks,ileft_peaks] = findpeaks(lshankfilt(ipk(p):ipk(p+1)));
    ...
    % update
    if skip
        warning('No peak was observed for either the right nor the left shank acceleration trace. Corresponding index will be 0.)')
    else
        fc(p) = fc(p) + ipk(p) - 1;
    end
